...
چی بکاپ میگیره:
  ...
  - تا ۳ #!/usr/bin/env python3
"""
customer_list.py

ساخت PDF از لیست مشتریان تلگرام (اکانت کاری) — فقط اسم، یوزرنیم، شماره، عکس.
...
راه‌اندازی یک‌باره:
  از my.telegram.org یه API ID و API Hash بگیر و پایین تو SETTINGS وارد کن.

اجرا:
  python3 customer_list.py
  python3 customer_list.py --pdf-only   (اگه contacts.json از قبل هست)
"""
...
def make_card(contact, font):
    ...
    if not in_contacts:
        info_block.append(Paragraph(rtl("(از چت‌ها)"), tag_s))
    if username:
...
def build_pdf(contacts, output: Path):
    ...
    def header(canvas, doc):
        ...
        canvas.drawRightString(A4[0]-1.5*cm, A4[1]-1.4*cm,
                               rtl("لیست مشتریان  —  " + str(total) + " نفر"))
...
def main():
    ...
    parser.add_argument("--pdf-only", action="store_true",
                        help="فقط PDF بساز از contacts.json موجود")
